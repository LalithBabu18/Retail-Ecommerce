<template>
  ...
  <span ><button type= "button" class="btn btn-success btn-md pull-right" :id="1" @click="$bvModal.show(1)" >Add SubCategory</button></span>
...
<div>
 <b-modal :id="1" hide-footer>
        ...
        <b-button class="mt-3" block @click="addCategory">Add SubCategory</b-button>
        </b-modal>
</div>
  ...
  <tbody >
    <tr  v-for="(item , index) in filteredFacility" :key="item.id"> -->
      ...
      <td>                      <i  class="fa fa-plus" aria-hidden="true" style="color:green" :value="item.id" @click="edit(item.id)"></i>
</td>
 ...
 <b-modal :id="item.id" hide-footer>
        ...
        <b-button class="mt-3" block @click="updateCategory(item.id)">Update Category</b-button>
        </b-modal>
</div>
    </tr>
  </tbody> -->
...
<div class="form-group" v-for="(input,k) in inputs" :key="k">
    ...
    <!-- <h1>{{}}</h1> -->
    <div v-for="k in dropArray" :key="k">
...
export default {
  ...
  data() {
    ...
    if (!data.error) {
      ...
      const dropArray = this.dropArray
      dropArray.push(data)
      dropArray.push(data)
...
`,
    ...
    if (!data.error) {
      ...
      if (confirm("are you sure?")) {
        ...
        await fetch(`http://localhost:8085/api/Category/${id}/deleteSubCategoryBySubCategoryId`, postBody).then(
          async() => {
             const auth_token = await localforage.getItem("my_access_token");
    const postBody = {
      method: "GET",
