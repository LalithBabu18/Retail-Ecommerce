...
export default {
  ...
  methods: {
    ...
    async addItems(subid, catid) {
      ...
      console.log(postBody);
      const response = fetch(
        `http://localhost:8085/api/Category/${catid}/addItems/${subid}`,
        postBody
      )
        // console.log(response, "Added");
        .then(async () => {
    ...
    async onChange() {
      ...
      const response = await fetch(
        `http://localhost:8085/api/Category/${this.selected}/getSubCategoryByCategoryId`,
    ...
    async onChange2() {
      ...
      const response = await fetch(
        `http://localhost:8085/api/Category/${this.selectedSub}/getItemsBySubCategoryId `,
    ...
    onFileChange() {
      ...
      reader.onload = async e => {
        ...
        const response = await fetch(
          `http://localhost:8085/api/Category/${this.selectedItem}/addStock/${this.selected}/${this.selectedSub} `,
          postBody
        );
    ...
    async addSubCategory(id) {
      ...
      console.log(postBody);
      const response = fetch(
        `http://localhost:8085/api/Category/${id}/addSubCategory`,
        postBody
      )
        // console.log(response, "Added");
        .then(() => {
    ...
    async deleteItem(id) {
      ...
      if (confirm("are you sure?")) {
        ...
        await fetch(
          `http://localhost:8085/api/Category/${id}/deleteItemsByItemsId `,
          postBody
        ).then(async () => {
          userReports.getList().then(resp => {
            if (!resp.error) {
